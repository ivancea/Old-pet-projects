...
public:
    mapa(int tamX, int tamY, int pixelSize, double x, double y, double grav, double maxVelocity=0, int blocks=1, int type=0)
        :_tamX(tamX),_tamY(tamY),_pixelSize(pixelSize), _type(type),
         ...
         _maxVelocity(maxVelocity),_player(sf::Color(0,0,0)),
    ...
    sf::Color getWallsColor()const{return _walls;}
    sf::Color getPlayerColor()const{return _player;}
    sf::Color getBackgroundColor()const{return _background;}
...
uint32_t getSerialValue(uint8_t media=1){
    uint32_t times = 0;
    uint32_t total = 0;
    se.WriteData(string(" "));
    while(true){
        string t = se.ReadDataLength(2,0);
...
void setNewBGColor(mapa &m){
    static sf::Color target(m.getBackgroundColor());

    sf::Color mp(m.getBackgroundColor());

    if(target == mp){
        target = sf::Color(rand()%200+25,rand()%200+25,rand()%200+25);
    }else{
        if(min(target.r, mp.r)+1<=max(target.r, mp.r))
            mp.r = mp.r+abs(target.r-mp.r)/(target.r-mp.r);
        if(min(target.g, mp.g)+1<=max(target.g, mp.g))
            mp.g = mp.g+abs(target.g-mp.g)/(target.g-mp.g);
        if(min(target.b, mp.b)+1<=max(target.b, mp.b))
            mp.b = mp.b+abs(target.b-mp.b)/(target.b-mp.b);
        m.setBackgroundColor(mp);
    }
}
...
int main(){
    ...
    int option = 0;
    {
        ...
        if(se.IsConnected()) op.addOption("Control analogico");
        option = op.show();
    }
    ...
    int blocks = 8, type=0, movimiento=0;
    uint32_t contador=0, cont2=PIXEL*10;
    bool muerto = false, justDied=false, space=true;
    ...
    while (window.isOpen()){
        ...
        if(!muerto && option==3)
            m.setY(getSerialValue()/(1024.0/double(MAPY*PIXEL)));
        sf::Event event;
        if (window.pollEvent(event)){
            ...
            case sf::Event::KeyPressed:
                ...
                if(option==0 && !muerto) m.setVelocity(m.getVelocity()-MAXVELOCITY*3/4);
